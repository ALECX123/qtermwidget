...
#  define MAP_FAILED ((char*)nullptr)
#else
#  include <sys/types.h>
#  include <sys/mman.h>
#  include <unistd.h>
...
void HistoryFile::map()
{
...
#if defined(Q_OS_WIN)
    // Windows CreateFileMapping 替代mmap
    HANDLE hFile = reinterpret_cast<HANDLE>(_get_osfhandle(ion));
    ...
    // 创建文件映射对象
    mapFileHandle = CreateFileMappingW(hFile, nullptr, PAGE_READONLY, 0, static_cast<DWORD>(length), nullptr);
    ...
    // 映射只读视图
    mapView = MapViewOfFile(mapFileHandle, FILE_MAP_READ, 0, 0, static_cast<SIZE_T>(length));
    if (mapView == nullptr)
    {
    ...
    fileMap = static_cast<char*>(mapView);
#else
    // Linux/macOS 原生mmap
    fileMap = static_cast<char*>(mmap(nullptr, static_cast<size_t>(length), PROT_READ, MAP_PRIVATE, ion, 0));
...
void HistoryFile::unmap()
{
    ...
    if (fileMap != nullptr)
    {
        int result = munmap(fileMap, static_cast<size_t>(length));
        Q_ASSERT(result == 0);
        Q_UNUSED(result);
    }
#endif
...
void HistoryFile::add(const unsigned char* bytes, int len)
{
    ...
    int rc = 0;
#if defined(Q_OS_WIN)
    rc = KDE_lseek(ion, static_cast<long>(length), SEEK_SET);
#else
    rc = KDE_lseek(ion, length, SEEK_SET);
#endif
    if (rc < 0) { perror("HistoryFile::add.seek"); return; }
...
#if defined(Q_OS_WIN)
        rc = KDE_lseek(ion, long(loc), SEEK_SET);
#else
        rc = KDE_lseek(ion, loc, SEEK_SET);
#endif
        if (rc < 0) { perror("HistoryFile::get.seek"); return; }
